...
export default class PaymentDrawer extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  submit = () => {
    const { user_type, roles, if_empty } = this.state.config;
    ...
    switch (user_type) {
      case 1:
        // 如果选中列表和全部出纳人员数组长度相同，即为全选中
        if (this.state.checkedList.length == this.state.paymentMembers.length) {
          config.users = null
          config.user_type = 1
        } else {
          let checkedMeber = this.state.paymentMembers.filter(member => { return this.state.checkedList.some(i => i == member.uid && !member.leave) })
          if (checkedMeber.length == 0) {
            Message.warn('请选择指定审核人')
            return;
          }
    ...
    config.pay_status = this.state.config.pay_status
  ...
  onRadioChange = (e, field) => {
    let checkedList = []
    if (e.target.value == 1) {
      checkedList = this.state.paymentMembers.map(item => item.uid)
    }
  ...
  render() {
    const { titleEditable, drawerModel, config, paymentMembers, treeData, memberSelectorVisible, checkedList, users, roleSelectorVisible } = this.state
    let roles = [];
    config.roles && (
      ...
      <div style={{ height: '100vh', position: 'relative' }}>
        ...
        <div className="drawer-content-wrapper">
          ...
          <Radio.Group name="radiogroup" className="approval-type-radio-group" onChange={(e) => { this.onRadioChange(e, 'user_type') }} value={config.user_type}>
            <Radio className="approval-type-radio" value={1}>全部出纳人员</Radio>
          ...
          <Hidden visible={config.user_type == 1 && paymentMembers && paymentMembers.length == 0}>
            <div className="setting-member-tips">
              <span className="icon"></span>
              <span className="text">未设置出纳权限人员，请先前往</span>
          ...
          <Hidden visible={config.user_type == 1 && paymentMembers && paymentMembers.length > 0}>
            <div>
              <Checkbox.Group onChange={this.onCheckboxGroupChange} value={checkedList}>
                {paymentMembers.map((item => {
